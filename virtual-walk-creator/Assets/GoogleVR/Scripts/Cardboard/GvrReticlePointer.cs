    ...
    public override void OnPointerHover(RaycastResult raycastResultResult, bool isInteractive)
    {
        SetPointerTarget(raycastResultResult.worldPosition, isInteractive);
            if(gazedAt != null && gazeStartTime > 0f){
                if(ExecuteEvents.CanHandleEvent<TimedInputHandler>(gazedAt)){
                    currentTime = Time.time - gazeStartTime;
                    if (currentTime > 3.0f){
                        gazeStartTime = -1f;
                    ...
                    {
                        _loader.Play(currentTime / 3.0f);
                    }
                }
                else if (ExecuteEvents.CanHandleEvent<InputHandler>(gazedAt)){
                    ExecuteEvents.Execute(gazedAt, null, (InputHandler handler, BaseEventData data) => handler.HandleInput() );
                }
            }
